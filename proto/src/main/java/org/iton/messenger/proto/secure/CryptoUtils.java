package org.iton.messenger.proto.secure;

import org.iton.messenger.proto.secure.aes.IAES256IGE;
import org.iton.messenger.proto.secure.aes.AES256IGE;
...
public class CryptoUtils {
    ...
    private static IAES256IGE currentImplementation = new AES256IGE();

    public static void setAESImplementation(IAES256IGE implementation) {
        currentImplementation = implementation;
    }

    public static byte[] RSAEncrypt(byte[] src, BigInteger key, BigInteger exponent) {
        try {
    ...
    public static byte[] RSADecrypt(byte[] src, BigInteger key, BigInteger exponent) {
        try {
            KeyFactory keyFactory = KeyFactory.getInstance("RSA");
            PrivateKey privateKey = keyFactory.generatePrivate(new RSAPrivateKeySpec(key, exponent));
            Cipher cipher = Cipher.getInstance("RSA/ECB/NoPadding");
    ...
    public static byte[] SHA256(byte[] src, int offset, int len) {
        MessageDigest crypt = sha256.get();
        crypt.reset();
        crypt.update(src, offset, len);;
        return crypt.digest();
    }
    ...
    public static byte[] convert(String in)
    {
        return DatatypeConverter.parseHexBinary(in);
    }

    public static String convert(byte[] in)
    {
